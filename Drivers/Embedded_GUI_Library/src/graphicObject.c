...
/* External variables --------------------------------------------------------*/
extern  uint32_t u32_TSXCoordinate;
extern  uint32_t u32_TSYCoordinate;
...
static uint16_t PageCount = 0;
 ...
 uint32_t vu32_gTimeOutCount = 0;
 uint8_t vu8_gTouchEnable = 1;
 uint8_t vu8_gSleepState = 0;
uint8_t ValidTouchDetected = 0;
static uint32_t _light_timeout_off = 10000;
static uint32_t _light_deep_off = 10;
static uint32_t _light_deep_on = 100;
/* Set to 0 to disable The Automatic Backlight Switch Off */
 uint8_t vu8_gPowerSaveOption = 1;
...
static GL_ErrStatus Create_IconBtn (GL_IconBtn_TypeDef* );
static GL_ErrStatus Create_IconBtnText (GL_IconBtnText_TypeDef* );
static GL_ErrStatus Create_IconBtnPushPullText (GL_IconBtnPushPullText_TypeDef* );
static GL_ErrStatus Create_CustomBtn (GL_CustomBtn_TypeDef* );
...
static GL_ErrStatus Create_Line (GL_Line_TypeDef* );
static GL_ErrStatus Create_ProgressBar (GL_ProgressBar_TypeDef* );
static GL_ErrStatus Create_Rect (GL_Rect_TypeDef* );
...
static GL_ErrStatus SetIconBtnVisible(GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
static GL_ErrStatus SetIconBtnTextVisible(GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
static GL_ErrStatus SetIconBtnPushPullTextVisible(GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
...
static GL_ErrStatus SetLineVisible( GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
static GL_ErrStatus SetProgressBarVisible( GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
static GL_ErrStatus SetRectVisible( GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
static GL_ErrStatus SetCustomBtnVisible( GL_PageControls_TypeDef* , GL_Coordinate_TypeDef );
...
static void GL_DrawRectangle(uint16_t maxX, uint16_t minX, uint8_t maxY, uint8_t minY)
{
  GL_DrawLine( maxX, minY,maxX - minX, GL_Horizontal);
...
static void GL_DrawObliqueLine(uint16_t Ypos1, uint16_t Xpos1, uint16_t Ypos2, uint16_t Xpos2, uint16_t Color)
{
  ...
  else if (Xpos1 == Xpos2)
  {
    ...
    GL_DrawFilledRectangle( Ypos2 - 1, Ypos1 - 1, Xpos2, Xpos1 , Color);
  }
  /*The general line*/
  else
  {
...
/**
  ...
  * @retval None
  */
void GL_DrawButtonBMP(uint16_t minX, uint16_t minY, uint16_t maxX, uint16_t maxY, uint8_t* ptrBitmap)
{
	LCD_Change_Direction(_90_degree);
	LCD_DrawColorBMP(ptrBitmap, minX, minY,maxX, maxY);
...
/**
  ...
  */
GL_PageControls_TypeDef* NewLabel (
    uint16_t ID,
    const uint8_t* label,
     ...
     uint32_t BackgroundColour)
{
  ...
  if (pControlObj)
  {
    ...
    pControlObj->Font = Font;
    pControlObj->FontColour = FontColour;
		pControlObj->BackgroundColour = BackgroundColour;
...
/**
  ...
  */
GL_PageControls_TypeDef* NewButton (uint16_t ID, const uint8_t* label, uint16_t Font, void (*pEventHandler)(void))
{
  GL_PageControls_TypeDef *pPageControlObj = NULL;
...
/**
  ...
  * @retval GL_PageControls_TypeDef* - The created Object pointer
  */
GL_PageControls_TypeDef* NewIconBtn (uint16_t ID, const uint8_t* Image_PTR, const uint8_t* Image_Press_PTR, uint16_t Width, uint8_t Height, void (*pEventHandler)(void))
{
  GL_IconBtn_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef* pPageControlObj = NULL;

  pControlObj = (GL_IconBtn_TypeDef *)malloc(sizeof(GL_IconBtn_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    pControlObj->ImagePTR = (uint8_t*)Image_PTR;
    pControlObj->ImagePressPTR = (uint8_t*)Image_Press_PTR;
    pControlObj->ImageWidth = Width;
    pControlObj->ImageHeight = Height;
    pControlObj->EventHandler = pEventHandler;

    /* Create the Icon object */
    Create_IconBtn(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_ICONBTN;
    }
    else
    {
      free(pControlObj);
...
{
  GL_IconBtnText_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef* pPageControlObj = NULL;

  pControlObj = (GL_IconBtnText_TypeDef *)malloc(sizeof(GL_IconBtnText_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    pControlObj->ImagePTR = (uint8_t*)Image_PTR;
    ...
    pControlObj->ImageWidth = Width;
    pControlObj->ImageHeight = Height;
		pControlObj->pText = pText;
		pControlObj->Font = Font;
		pControlObj->FontColour = FontColour;
    pControlObj->EventHandler = pEventHandler;

    /* Create the Icon object */
    Create_IconBtnText(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_ICON_BTN_TEXT;
    }
    else
    {
      free(pControlObj);
...
GL_PageControls_TypeDef* NewIconBtnPushPullText (
			...
			void (*pEventHandler)(void))
{
  GL_IconBtnPushPullText_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef* pPageControlObj = NULL;

  pControlObj = (GL_IconBtnPushPullText_TypeDef *)malloc(sizeof(GL_IconBtnPushPullText_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    pControlObj->ImagePTR = (uint8_t*)Image_PTR;
    ...
    pControlObj->ImageHeight = Height;
		pControlObj->pText = pText;
		pControlObj->PushPullState = PushPullState;
    pControlObj->EventHandler = pEventHandler;

    /* Create the Icon object */
    Create_IconBtnPushPullText(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_ICON_BTN_PUSH_PULL_TEXT;
    }
    else
    {
      free(pControlObj);
...
{
  GL_IconBtnPushPullText_TypeDef* pTmp;
  uint32_t index = 0;
  ...
  /* search for the required label */
    ...
    {
      if (pPage->PageControls[index]->objType == GL_ICON_BTN_PUSH_PULL_TEXT)
      {
        pTmp = (GL_IconBtnPushPullText_TypeDef*)(pPage->PageControls[index]->objPTR);

        if ( pTmp->ID == ID)
        {
					pTmp->PushPullState = 0;
          return GL_OK;
        }
      }
      index++;
...
/**
  ...
  * @retval GL_PageControls_TypeDef* - The created Object pointer
  */
GL_PageControls_TypeDef* NewGraphChart ( uint16_t ID, const uint8_t* labelX, const uint8_t* labelY, int16_t data_points[], uint32_t n_points, GL_bool Background )
{
  GL_PageControls_TypeDef *pPageControlObj = NULL;
...
/**
  ...
  * @retval pPageControlObj - The pointer to the graphic object
  */
GL_PageControls_TypeDef* NewLine (uint32_t ID,  uint32_t Len, GL_Direction direction,  uint16_t Colour)
{
  GL_Line_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef * pPageControlObj = NULL;

  pControlObj = (GL_Line_TypeDef *)malloc(sizeof(GL_Line_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    ...
    /* Create the Label object */
    Create_Line(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_LINE;
    }
    else
    {
      free(pControlObj);
...
GL_PageControls_TypeDef* NewCustomBtn (
			...
			void (*pEventHandler)(void))
{
  GL_CustomBtn_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef * pPageControlObj = NULL;

  pControlObj = (GL_CustomBtn_TypeDef *)malloc(sizeof(GL_CustomBtn_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    pControlObj->Color = Color;
    pControlObj->ColorPress = ColorPress;
    ...
    /* Create the Label object */
    Create_CustomBtn(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_CUSTOM_BUTTON;
    }
    else
    {
      free(pControlObj);
...
/**
  ...
  * @retval pPageControlObj - The pointer to the graphic object
  */
GL_PageControls_TypeDef* NewRect (uint16_t ID,  uint16_t LenX,  uint16_t LenY,  uint16_t Colour)
{
  GL_Rect_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef * pPageControlObj = NULL;

  pControlObj = (GL_Rect_TypeDef *)malloc(sizeof(GL_Rect_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;

    pControlObj->Colour = Colour;
		pControlObj->LenX  = LenX;
		pControlObj->LenY  = LenY;

    /* Create the Label object */
    Create_Rect(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_RECT;
    }
    else
    {
      free(pControlObj);
...
/**
  ...
  * @retval pPageControlObj - The pointer to the graphic object
  */
GL_PageControls_TypeDef* NewProgressBar (uint16_t ID,  uint16_t Len,  uint16_t Value, GL_Direction direction,  uint16_t Colour)
{
  GL_ProgressBar_TypeDef *pControlObj = NULL;
  GL_PageControls_TypeDef * pPageControlObj = NULL;

  pControlObj = (GL_ProgressBar_TypeDef *)malloc(sizeof(GL_ProgressBar_TypeDef));
  if (pControlObj)
  {
    pControlObj->ID = ID;
    ...
    pControlObj->Direction = direction;
		pControlObj->Len  = Len;
		pControlObj->Value= Value;

    /* Create the Label object */
    Create_ProgressBar(pControlObj);

    pPageControlObj = (GL_PageControls_TypeDef*)malloc(sizeof(GL_PageControls_TypeDef));
    if ( pPageControlObj )
    {
      pPageControlObj->objPTR = (void*)pControlObj;
      pPageControlObj->objType = GL_PROGRESSBAR;
    }
    else
    {
      free(pControlObj);
...
/**
  ...
  */
GL_ErrStatus AddPageControlObj (uint32_t PosX, uint32_t PosY, GL_PageControls_TypeDef* objPTR, GL_Page_TypeDef* pagePTR)
{
  if (pagePTR->ControlCount < MAX_CTRL_X_PAGE)
  {
    ...
    else
    {
      ...
      else if (objPTR->objType == GL_ICONBTN)
      {
				GL_IconBtn_TypeDef* pTmp = ((GL_IconBtn_TypeDef*)(objPTR->objPTR));
        objPTR->SetObjVisible = SetIconBtnVisible;
        objPTR->ID = pTmp->ID;
        objCoordinates.MaxX = PosX + pTmp->ImageWidth;
        objCoordinates.MaxY = PosY + pTmp->ImageHeight;
      }
      else if (objPTR->objType == GL_ICON_BTN_TEXT)
      {
				GL_IconBtnText_TypeDef* pTmp = ((GL_IconBtnText_TypeDef*)(objPTR->objPTR));
        objPTR->SetObjVisible = SetIconBtnTextVisible;
        objPTR->ID = pTmp->ID;
        objCoordinates.MaxX = PosX + pTmp->ImageWidth;
        objCoordinates.MaxY = PosY + pTmp->ImageHeight;
      }
      else if (objPTR->objType == GL_ICON_BTN_PUSH_PULL_TEXT)
      {
				GL_IconBtnPushPullText_TypeDef* pTmp = ((GL_IconBtnPushPullText_TypeDef*)(objPTR->objPTR));
        objPTR->SetObjVisible = SetIconBtnPushPullTextVisible;
        objPTR->ID = pTmp->ID;
        objCoordinates.MaxX = PosX + pTmp->ImageWidth;
        objCoordinates.MaxY = PosY + pTmp->ImageHeight;
      }
      ...
      else if(objPTR->objType == GL_GRAPH_CHART)
      {
        ...
        objPTR->ID = pTmp->ID;
			...
			else if (objPTR->objType == GL_LINE)
			{
				GL_Line_TypeDef* pTmp = ((GL_Line_TypeDef*)(objPTR->objPTR));
				objPTR->SetObjVisible = SetLineVisible;
				objPTR->ID = pTmp->ID;
				objCoordinates.MaxX = PosX;
				objCoordinates.MaxY = PosY;
				//TODO:coord
			}
			else if (objPTR->objType == GL_RECT)
			{
				GL_Rect_TypeDef* pTmp = ((GL_Rect_TypeDef*)(objPTR->objPTR));
				objPTR->SetObjVisible = SetRectVisible;
				objPTR->ID = pTmp->ID;
				objCoordinates.MaxX = PosX; //+ pTmp->LenX;
				objCoordinates.MaxY = PosY;// + pTmp->LenY;
			}
			else if (objPTR->objType == GL_PROGRESSBAR)
			{
				GL_ProgressBar_TypeDef* pTmp = ((GL_ProgressBar_TypeDef*)(objPTR->objPTR));
				objPTR->SetObjVisible = SetProgressBarVisible;
				objPTR->ID = pTmp->ID;
				objCoordinates.MaxX = PosX + pTmp->Len;
				objCoordinates.MinX = PosX ;
				objCoordinates.MinY = PosY;
				objCoordinates.MaxY = PosY + 15;
			}
			else if (objPTR->objType == GL_CUSTOM_BUTTON)
			{
				GL_CustomBtn_TypeDef* pTmp = ((GL_CustomBtn_TypeDef*)(objPTR->objPTR));
        objPTR->SetObjVisible = SetCustomBtnVisible;
        objPTR->ID = pTmp->ID;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
GL_ErrStatus Set_Label_Colour( GL_Page_TypeDef* pPage, uint16_t ID, uint32_t FontColour,  uint32_t BackgroundColour )
{
  GL_Label_TypeDef* pTmp;
  uint32_t index = 0;
  ...
  if (!pPage)
  {
    ...
    while ( index < pPage->ControlCount )
    {
      if (pPage->PageControls[index]->objType == GL_LABEL)
      {
        ...
        if ( pTmp->ID == ID)
        {
          pTmp->FontColour = FontColour;
					pTmp->BackgroundColour = BackgroundColour;
          return GL_OK;
        }
      }
      index++;
			...
			const uint16_t FontColour)
{
  ...
  while ( index < pPage->ControlCount ) /* search for the required label */
  {
    if (pPage->PageControls[index]->objType == GL_CUSTOM_BUTTON)
    {
      pTmp = (GL_CustomBtn_TypeDef*)(pPage->PageControls[index]->objPTR);
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_IconBtn (GL_IconBtn_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_IconBtnText (GL_IconBtnText_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_IconBtnPushPullText (GL_IconBtnPushPullText_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_CustomBtn (GL_CustomBtn_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_Rect(GL_Rect_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_Line (GL_Line_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
/**
  * @brief  Assign ProgressBar Methods/Properties members.
  * @param  *pThis: Pointer to Object Structure
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus Create_ProgressBar (GL_ProgressBar_TypeDef* pThis)
{
  if (!pThis)
  {
    return GL_ERROR;
...
GL_ErrStatus Create_PageObj (GL_Page_TypeDef* pThis, void (*TimerHandler)())
{
  ...
  else
  {
    ...
    pThis->ControlCount      = 0;
		pThis->TimerHandler      = TimerHandler;
...
static GL_ErrStatus SetLabelVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  {
    GL_SetTextColor(pThis->FontColour);
		if (pThis->BackgroundColour != GL_Transparent)
		{
			GL_SetBackColor(pThis->BackgroundColour);

      FONT_SetFont(pThis->Font);
      if ( pThis->Direction == GL_HORIZONTAL )
      {
        GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MinX), (uint16_t)(objCoordinates.MinY),
                                 (uint8_t*)pThis->label,
                                 GL_FALSE);
      }
      else if ( pThis->Direction == GL_LEFT_VERTICAL )
      {
        ...
        GL_DisplayAdjStringLine( (uint16_t)(LCD_Width - objCoordinates.MinY),
                             (uint16_t)(objCoordinates.MaxX + p_strlen((pThis->label))/*GL_FONT_SMALL_WIDTH*/),
                             (uint8_t*)pThis->label,GL_FALSE);
        LCD_Change_Direction(_0_degree);
      }
      else if ( pThis->Direction == GL_RIGHT_VERTICAL )
      {
        LCD_Change_Direction(_90_degree);
        GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MinY),
  ...
  if (pThis->isObjectTouched == GL_FALSE)
  {
    ...
    if (LabelLength>MAX_BUTTON_LABEL_LENGTH)
      GL_DrawFilledRectangle((uint16_t)(objCoordinates.MaxX),
                             ...
                             (uint16_t)(objCoordinates.MinY), GL_Blue);
    else
      GL_DrawFilledRectangle((uint16_t)(objCoordinates.MaxX),
  ...
  else if (pThis->isObjectTouched == GL_TRUE)
  {
    ...
    if (LabelLength>MAX_BUTTON_LABEL_LENGTH)
      GL_DrawFilledRectangle((uint16_t)(objCoordinates.MaxX),
                             (uint16_t)(objCoordinates.MaxX-((MAX_BUTTON_LABEL_LENGTH-1)*FONT_LENGTH)),
                             (uint16_t)(objCoordinates.MaxY),
                             (uint16_t)(objCoordinates.MinY), GL_Grey);
    else
      GL_DrawFilledRectangle((uint16_t)(objCoordinates.MaxX),
                             (uint16_t)(objCoordinates.MaxX-((LabelLength+1)*FONT_LENGTH)),
                             (uint16_t)(objCoordinates.MaxY),
...
static GL_ErrStatus SetSwitchVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX),
                    (uint16_t)(objCoordinates.MaxX - BUTTON_SLICE_LENGTH),
                    (uint16_t)(objCoordinates.MaxY),
                    (uint16_t)(objCoordinates.MinY), ptrBitmapLeft );
  for ( ; n < maxLabelLength && n < MAX_SWITCH_LABEL_LENGTH; n++)
  {
    GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX - (n*BUTTON_SLICE_LENGTH)),
                      (uint16_t)(objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)),
                      (uint16_t)(objCoordinates.MaxY),
                      (uint16_t)(objCoordinates.MinY), ptrBitmapCenter );
  }
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX - (n*BUTTON_SLICE_LENGTH)),
                    (uint16_t)(objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)),
                    (uint16_t)(objCoordinates.MaxY),
                    (uint16_t)(objCoordinates.MinY), ptrBitmapRight );
#endif
  GL_SetTextColor(GL_White);
...
static GL_ErrStatus SetRadioButtonVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  if ( pThis->IsChecked == GL_TRUE )
  {
...
#else
    GL_SetTextColor( GL_Black );
    GL_DrawFilledCircle( (uint16_t)(objCoordinates.MaxX), (uint16_t)(objCoordinates.MaxY), RADIO_BUTTON_RADIUS, GL_White );
    GL_DrawFilledCircle( (uint16_t)(objCoordinates.MaxX-(RADIO_BUTTON_RADIUS/2 -1)),
                         (uint16_t)(objCoordinates.MaxY-(RADIO_BUTTON_RADIUS/2 -1)),
    ...
    GL_DrawFilledRectangle( (uint16_t)(objCoordinates.MaxX-(RADIO_BUTTON_RADIUS/2)-2),
                            ...
                            (uint16_t)(objCoordinates.MaxY-(RADIO_BUTTON_RADIUS/2)-2),
                            (uint16_t)(objCoordinates.MinY+(RADIO_BUTTON_RADIUS/2)+4), GL_Black);
#endif
  }
  else
  {
    ...
    GL_DrawFilledRectangle( (uint16_t)(objCoordinates.MaxX-(RADIO_BUTTON_RADIUS/2)-2),
                               (uint16_t)(objCoordinates.MinX+(RADIO_BUTTON_RADIUS/2)+4),
                               (uint16_t)(objCoordinates.MaxY-(RADIO_BUTTON_RADIUS/2)-2),
                               (uint16_t)(objCoordinates.MinY+(RADIO_BUTTON_RADIUS/2)+4), GL_White);
#endif
  }
  ...
  {
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MaxY) - (RADIO_BUTTON_RADIUS-2)*2,
...
static GL_ErrStatus SetComboBoxVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  GL_DrawFilledRectangle( (uint16_t)(objCoordinates.MaxX),
                          (uint16_t)(objCoordinates.MaxX-COMBOBOX_SIZE),
                          (uint16_t)(objCoordinates.MaxY),
                          (uint16_t)(objCoordinates.MinY+1), GL_Grey);
#endif
  ...
  if (pThis->Secondary == GL_FALSE)
  {
    ...
    FONT_SetFont(GL_FONT_BIG);
    GL_SetTextColor(GL_Black);
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MinY-1), (uint16_t)(objCoordinates.MaxX-1), "-", GL_TRUE);
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MinY-1), (uint16_t)(objCoordinates.MaxX-4), "-", GL_TRUE);
#endif
    ...
    GL_DrawFilledRectangle( (uint16_t)(objCoordinates.MinX-COMBOBOX_SIZE),
                            ...
                            (uint16_t)(objCoordinates.MaxY),
                            (uint16_t)(objCoordinates.MinY+1), GL_White );
    FONT_SetFont(FONT_DEFAULT);
    GL_SetTextColor(GL_Blue);
    if(pThis->ComboOptionCount>0)
    {
      GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MaxY - 16),
  ...
  else
  {
    ...
    FONT_SetFont(GL_FONT_BIG);
    GL_SetTextColor(GL_Black);
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MinY), (uint16_t)(objCoordinates.MaxX-3), "+", GL_TRUE);
#endif
    ...
    GL_DrawFilledRectangle( (uint16_t)(objCoordinates.MinX),
                            ...
                            (uint16_t)(objCoordinates.MaxY),
                            (uint16_t)(objCoordinates.MinY+1),
                            GL_White );
    FONT_SetFont(FONT_DEFAULT);
    GL_SetTextColor(GL_Blue);
...
static GL_ErrStatus SetCheckboxVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  else
  {
    ...
    GL_DrawFilledRectangle((uint16_t)(objCoordinates.MaxX-1),
                           ...
                           (uint16_t)(objCoordinates.MinY+1), GL_White);
    GL_SetTextColor(GL_Black);
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MaxY)-19, (uint16_t)(objCoordinates.MaxX-2), "X", GL_TRUE);
#endif
  }
#ifndef USE_2D_OBJECTS
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX),
  ...
  {
    GL_DisplayAdjStringLine( (uint16_t)(objCoordinates.MaxY) - 15,
...
static GL_ErrStatus SetIconVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MinX),
                    (uint16_t)(objCoordinates.MinY),
                    (uint16_t)(objCoordinates.MaxX),
                    (uint16_t)(objCoordinates.MaxY),
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetIconBtnVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_IconBtn_TypeDef* pThis = (GL_IconBtn_TypeDef*)(pTmp->objPTR);
  if (!pThis)
  {
    return GL_ERROR;
  }
  ...
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX),
	...
	{
		if (pThis->ImagePressPTR)
		{
			GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX),
                    (uint16_t)(objCoordinates.MinX),
                    (uint16_t)(objCoordinates.MaxY),
                    (uint16_t)(objCoordinates.MinY),
                    (uint8_t*)pThis->ImagePressPTR );
		}
	}
  return GL_OK;
}
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetIconBtnTextVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_IconBtnText_TypeDef* pThis = (GL_IconBtnText_TypeDef*)(pTmp->objPTR);
	uint8_t temp_string[16];
	uint32_t str_pix_size,
		n_pos = 0,
		total_pos = 0,
		i = 0,
		offset=0,
  ...
  GL_DrawButtonBMP( (uint16_t)(objCoordinates.MinX),
	...
	if (pThis->pText)
	{
		...
		{
			n_pos = 0;
			while(n_pos < 16 && pThis->pText[n_pos + total_pos] != '\n' && pThis->pText[n_pos + total_pos] != 0 )
			{
				n_pos++;
			}
			memcpy(temp_string, (uint8_t*)&pThis->pText[total_pos],n_pos);
			temp_string[n_pos] = 0;
			total_pos += (n_pos + 1);
			str_pix_size = FONT_GetLineLenPix(pThis->Font,(uint8_t*)temp_string);

			GL_DisplayAdjStringLine(
				objCoordinates.MinX + ((objCoordinates.MaxX - objCoordinates.MinX) - str_pix_size)/2,
				objCoordinates.MinY + 16*i+offset,
        (uint8_t*)temp_string,
        GL_TRUE);
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetCustomBtnVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_CustomBtn_TypeDef* pThis = (GL_CustomBtn_TypeDef*)(pTmp->objPTR);
	uint8_t temp_string[16];
	uint32_t str_pix_size,
		n_pos = 0,
		total_pos = 0,
  ...
  pThis->Control_Visible = GL_TRUE;
	...
	GL_SetDisplayWindow(
		(uint16_t)objCoordinates.MinX,
		(uint16_t)objCoordinates.MinY + 3,
		objCoordinates.MaxX,
		objCoordinates.MaxY - 2, 0);

	if (!pThis->isObjectTouched)
	{
		GL_ClearWindow(pThis->Color);
		GL_SetTextColor(pThis->Color);
	}
	else
	{
		GL_ClearWindow(pThis->ColorPress);
		GL_SetTextColor(pThis->ColorPress);
	}

	GL_DrawLine(objCoordinates.MinX + 2, objCoordinates.MinY, pThis->Width - 4, GL_Horizontal);
	GL_DrawLine(objCoordinates.MinX + 1, objCoordinates.MinY + 1, pThis->Width - 2, GL_Horizontal);
	GL_DrawLine(objCoordinates.MinX + 1, objCoordinates.MinY + 2, pThis->Width - 2, GL_Horizontal);
	GL_DrawLine(objCoordinates.MinX + 1, objCoordinates.MaxY - 2, pThis->Width - 2, GL_Horizontal);
	GL_DrawLine(objCoordinates.MinX + 1, objCoordinates.MaxY - 1, pThis->Width - 2, GL_Horizontal);
	GL_DrawLine(objCoordinates.MinX + 2, objCoordinates.MaxY, pThis->Width - 4, GL_Horizontal);
	...
	if (pThis->pText)
	{
        ...
        FontHeight = (FONT_GetMaxHeight(pThis->Font) + 3/* pixel for space */);
        line_count_max = pThis->Height / FontHeight ;

        line_count = MIN(line_count, line_count_max);

        offset = (pThis->Height - FontHeight * line_count) / 2;
		...
		for (i=0;i<(line_count);i++)
		{
			...
			GL_DisplayAdjStringLine(
				objCoordinates.MinX + ((objCoordinates.MaxX - objCoordinates.MinX) - str_pix_size)/2,
				objCoordinates.MinY + FontHeight*i+offset,
                (uint8_t*)temp_string,
                GL_TRUE);
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetIconBtnPushPullTextVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_IconBtnPushPullText_TypeDef* pThis = (GL_IconBtnPushPullText_TypeDef*)(pTmp->objPTR);
	uint8_t temp_string[16];
	uint32_t str_pix_size,
		n_pos = 0,
		total_pos = 0,
  ...
  pThis->Control_Visible = GL_TRUE;
	...
	{
		pThis->PushPullState = !pThis->PushPullState ;
	}
	...
	if (pThis->pText)
	{
		...
		for (i=0;i<(n_count+1);i++)
		{
			...
			temp_string[n_pos] = 0;
			total_pos += (n_pos + 1);
			str_pix_size = FONT_GetLineLenPix(FONT_DEFAULT, (uint8_t*)temp_string);
			GL_DisplayAdjStringLine( objCoordinates.MinY + 16*i+offset,
															objCoordinates.MaxX - ((objCoordinates.MaxX - objCoordinates.MinX) - str_pix_size)/2,
                           (uint8_t*)temp_string,
                           GL_TRUE);
			if(n_pos == 15)
			{
...
static GL_ErrStatus SetSlidebarVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  ...
  if ( label_length > 0 )
  {
    ...
    else if (pThis->Direction == GL_LEFT_VERTICAL)
    {
      ...
      for (; counter < label_length - 1; counter++)
      {
        GL_DisplayAdjStringLine( LCD_Width - minY + 2,
...
static GL_ErrStatus SetGraphChartVisible( GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates )
{
  ...
  max_value = 0;
  max_x = LCD_Width -5;//- GRAPH_MARGIN_LENGTH * 5 / 2 - FONT_LENGTH;
  base_y =pTmp->objCoordinates.MinY + LCD_Height-120;// - GRAPH_MARGIN_LENGTH * 7 / 2;

  offset_x = max_x + 4;
  offset_y = base_y;

  width  = LCD_Width -5;// - GRAPH_MARGIN_LENGTH * 4;
  height = LCD_Height-120;// - GRAPH_MARGIN_LENGTH * 9 / 2;

  x_step = (double)(width) / (pThis->n_points);
  y_step = (double)(height / 10);

  GL_SetTextColor(GL_Black);
  FONT_SetFont(FONT_DEFAULT);
  GL_DrawLine( (uint16_t)max_x,  (uint16_t)base_y,(uint16_t)(width), GL_Horizontal );      /* X Axis */
  GL_DrawLine((uint16_t)max_x,  (uint16_t)(base_y - height), (uint16_t)height, GL_Vertical );  /* Y Axis */
  ...
  //GL_DisplayAdjStringLine( (uint16_t)(offset_y - 5), (uint16_t)(offset_x + GRAPH_MARGIN_LENGTH - 9), "0", GL_TRUE ); /* min Y value*/
	//GL_DisplayAdjStringLine( (uint16_t)(base_y + 0), (uint16_t)(max_x - 5), "0", GL_TRUE ); /* min X, Y value*/

  if (pThis->Background == GL_TRUE)
  {
    /* Display the grid background color (black) */
    GL_SetDisplayWindow((uint16_t)(base_y-1), (uint16_t)max_x-1, (uint16_t)height, (uint16_t)(width + 2),0);

    //GL_ClearWindow(GL_Black);
		GL_ClearWindow(GL_White);

    GL_SetDisplayWindow(LCD_Height - 1, LCD_Width - 1, LCD_Height, LCD_Width, 0);
    signal_color = 16;//GL_Green;
  }
	...
	for (index = 0; index <= 10; index+=2)
  {
    ...
    //if ( index == 10)
    {
      ...
      sprintf((char*)value, "%4.1f", (max_value/10)*index);
  ...
  for (index = 0; index < (pThis->n_points); index++)
  {
    ...
    else
    {
      ...
      if ((index + 1) % (pThis->n_points / 5) == 0)
      {

        x_point_val++;//=index + 1;
				sprintf((char*)value, "%d", x_point_val);
        GL_DisplayAdjStringLine( (uint16_t)(base_y + 2), (uint16_t)(max_x + 6), (uint8_t*) value, GL_TRUE );
        GL_SetTextColor(GL_Black);
        GL_DrawLine( (uint16_t)(max_x + 1), (uint16_t)(base_y - height), (uint16_t)height, GL_Vertical );
			}
		}
    ...
    if (index == 0)
    {
			LCD_PutPixel((uint16_t)max_x, (uint16_t)base_y, GL_Black, FirstPixel);
#if 0
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetRectVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_Rect_TypeDef* pThis = (GL_Rect_TypeDef*)(pTmp->objPTR);
  if (!pThis)
  {
    return GL_ERROR;
  }
  pThis->Control_Visible = GL_TRUE;
  if (pTmp->objType == GL_RECT)
  {
		GL_SetDisplayWindow((uint16_t)objCoordinates.MinX, (uint16_t)objCoordinates.MinY, objCoordinates.MaxX + pThis->LenX, objCoordinates.MaxY + pThis->LenY, 0);
		GL_ClearWindow(pThis->Colour);
  }
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetLineVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
  GL_Line_TypeDef* pThis = (GL_Line_TypeDef*)(pTmp->objPTR);
  if (!pThis)
  {
    return GL_ERROR;
  }
  pThis->Control_Visible = GL_TRUE;
  if (pTmp->objType == GL_LINE)
  {
		GL_SetTextColor(pThis->Colour);
		if ( pThis->Direction == GL_HORIZONTAL )
		{
			GL_DrawLine( objCoordinates.MinX, objCoordinates.MinY, pThis->Len, GL_HORIZONTAL);
    }
		else
		{
			GL_DrawLine(objCoordinates.MinX,objCoordinates.MinY, pThis->Len, GL_LEFT_VERTICAL);
		}
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
static GL_ErrStatus SetProgressBarVisible(GL_PageControls_TypeDef* pTmp, GL_Coordinate_TypeDef objCoordinates)
{
	uint32_t height=15, width_value, width;

  GL_ProgressBar_TypeDef* pThis = (GL_ProgressBar_TypeDef*)(pTmp->objPTR);
  if (!pThis)
  {
    return GL_ERROR;
  }
  pThis->Control_Visible = GL_TRUE;
	width = pThis->Len-2;
	width_value = (width * pThis->Value) / 100;
#ifdef USE_2D_OBJECTS
	...
	if (pThis->Value != 100)
	{
		GL_SetDisplayWindow((uint16_t)objCoordinates.MaxY-1, (uint16_t)objCoordinates.MaxX - width_value-1,(uint16_t)(height-1), (uint16_t)width - width_value, 0);
		GL_ClearWindow(GL_White);
	}

	GL_SetDisplayWindow((uint16_t)objCoordinates.MaxY-1, (uint16_t)objCoordinates.MaxX-1, (uint16_t)(height-1), (uint16_t)width_value, 0);
	GL_ClearWindow(pThis->Colour);
...
GL_ErrStatus SetProgressBarValue(GL_Page_TypeDef* pPage, uint16_t ID, uint16_t Value)
{
  ...
  {
    if (pPage->PageControls[index]->objType == GL_PROGRESSBAR)
    {
      GL_ProgressBar_TypeDef* pTmp;
      pTmp = (GL_ProgressBar_TypeDef*)(pPage->PageControls[index]->objPTR);
...
/**
  ...
  * @retval GL_ErrStatus - GL_OK if successful, GL_ERROR otherwise
  */
GL_ErrStatus SetGraphChartPoints( GL_Page_TypeDef* pPage, uint16_t ID, int16_t data_points[], uint32_t n_points )
{
  uint32_t index = 0;
  if (!pPage)
  {
...
static GL_ObjDimensions_TypeDef GetObjSize(GL_PageControls_TypeDef* pPageControl)
{
  ...
  else if (pPageControl->objType == GL_SLIDEBAR)
  {
	...
	else if (pPageControl->objType == GL_ICON)
	{
		GL_Icon_TypeDef* pImage;
    pImage = (GL_Icon_TypeDef*)(pPageControl->objPTR);
		if (pImage->Control_Visible)
		{
			dimensions.Length = pImage->ImageWidth;
			dimensions.Height = pImage->ImageHeight;
		}

	}
	else if (pPageControl->objType == GL_ICONBTN)
	{
		GL_IconBtn_TypeDef* pImage;
    pImage = (GL_IconBtn_TypeDef*)(pPageControl->objPTR);
		if (pImage->Control_Visible)
		{
			dimensions.Length = pImage->ImageWidth;
			dimensions.Height = pImage->ImageHeight;
	...
	else if (pPageControl->objType == GL_ICON_BTN_TEXT)
	{
		GL_IconBtnText_TypeDef* pImage;
    pImage = (GL_IconBtnText_TypeDef*)(pPageControl->objPTR);
		if (pImage->Control_Visible)
		{
			dimensions.Length = pImage->ImageWidth;
			dimensions.Height = pImage->ImageHeight;
	...
	else if (pPageControl->objType == GL_ICON_BTN_PUSH_PULL_TEXT)
	{
		GL_IconBtnPushPullText_TypeDef* pImage;
    pImage = (GL_IconBtnPushPullText_TypeDef*)(pPageControl->objPTR);
		if (pImage->Control_Visible)
		{
			dimensions.Length = pImage->ImageWidth;
			dimensions.Height = pImage->ImageHeight;
	...
	else if (pPageControl->objType == GL_CUSTOM_BUTTON)
	{
		GL_CustomBtn_TypeDef* pImage;
    pImage = (GL_CustomBtn_TypeDef*)(pPageControl->objPTR);
		if (pImage->Control_Visible)
		{
			dimensions.Length = pImage->Width;
			dimensions.Height = pImage->Height;
		}
...
static void CallPreEvents(GL_PageControls_TypeDef* pControl)
{
  ...
  switch (pControl->objType)
  {
    case GL_BUTTON:
      ...
      break;
		case GL_ICONBTN:
			pTmp = (GL_IconBtn_TypeDef*)(pControl->objPTR);
      ((GL_IconBtn_TypeDef*)(pTmp))->isObjectTouched = GL_TRUE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
      GL_Delay(250);
      ((GL_IconBtn_TypeDef*)(pTmp))->isObjectTouched = GL_FALSE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
			break;
		case GL_ICON_BTN_TEXT:
			pTmp = (GL_IconBtnText_TypeDef*)(pControl->objPTR);
      ((GL_IconBtnText_TypeDef*)(pTmp))->isObjectTouched = GL_TRUE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
      GL_Delay(250);
      ((GL_IconBtnText_TypeDef*)(pTmp))->isObjectTouched = GL_FALSE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
			break;
		case GL_ICON_BTN_PUSH_PULL_TEXT:
			pTmp = (GL_IconBtnPushPullText_TypeDef*)(pControl->objPTR);
      ((GL_IconBtnPushPullText_TypeDef*)(pTmp))->isObjectTouched = GL_TRUE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
      GL_Delay(250);
      ((GL_IconBtnPushPullText_TypeDef*)(pTmp))->isObjectTouched = GL_FALSE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
			break;
		case GL_CUSTOM_BUTTON:
			pTmp = (GL_CustomBtn_TypeDef*)(pControl->objPTR);
      ((GL_CustomBtn_TypeDef*)(pTmp))->isObjectTouched = GL_TRUE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
      GL_Delay(250);
      ((GL_CustomBtn_TypeDef*)(pTmp))->isObjectTouched = GL_FALSE;
      pControl->SetObjVisible(pControl, pControl->objCoordinates);
			break;
    case GL_CHECKBOX:
    ...
    case GL_COMBOBOX:
      ...
      GL_DrawButtonBMP( (uint16_t)(pControl->objCoordinates.MaxX),
                        ...
                        (uint16_t)(pControl->objCoordinates.MaxY),
                        (uint16_t)(pControl->objCoordinates.MinY), ptrBitmap );
      GL_Delay(20);
#else
      GL_DrawFilledRectangle( (uint16_t)(pControl->objCoordinates.MaxX),
                              (uint16_t)(pControl->objCoordinates.MaxX-COMBOBOX_SIZE),
                              (uint16_t)(pControl->objCoordinates.MaxY),
                              (uint16_t)(pControl->objCoordinates.MinY+1), GL_Cyan);
#endif
...
void ProcessInputData(void)
{
  ...
  if ( touch_done || joy_done )
  {
    ...
    {
      GL_BackLightSwitch(_light_deep_on);
      vu8_gSleepState = 0;
    }
      ...
      for ( p_index = 0; p_index < PageCount; p_index++)
      {
        if ( PagesList[p_index]->Page_Active == GL_TRUE )
        {
          for ( c_index = 0; c_index < PagesList[p_index]->ControlCount; c_index++ )
          {
            ...
            if (touch_done)
            {
							//CursorShow(u32_TSYCoordinate, u32_TSXCoordinate);
              //if ( CompareCoordinates( tmpCoord.MaxX, tmpCoord.MaxX - tmpSize.Length + 1, tmpCoord.MaxY, tmpCoord.MaxY - tmpSize.Height ) )
							if ( CompareCoordinates( tmpCoord.MaxX, tmpCoord.MinX, tmpCoord.MaxY, tmpCoord.MinY) )
              {
                //CursorDraw(Cursor->X, Cursor->Y, CUR_DRAW_BEH);
...
static void CallEvent(GL_PageControls_TypeDef* pControl)
{
  ...
  switch (pControl->objType)
  {
    case GL_BUTTON:
      pTmp = (GL_Button_TypeDef*)(pControl->objPTR);
			if(((GL_Button_TypeDef*)pTmp)->EventHandler)
			{
    ...
    case GL_CHECKBOX:
      pTmp = (GL_Checkbox_TypeDef*)(pControl->objPTR);
			if(((GL_Checkbox_TypeDef*)pTmp)->EventHandler)
			{
    ...
    case GL_SWITCH:
      pTmp = (GL_Switch_TypeDef*)(pControl->objPTR);
			if(((GL_Switch_TypeDef*)pTmp)->EventHandler)
			{
    ...
    case GL_ICON:
      pTmp = (GL_Icon_TypeDef*)(pControl->objPTR);
			if(((GL_Icon_TypeDef*)pTmp)->EventHandler)
			{
    ...
    case GL_ICONBTN:
      pTmp = (GL_IconBtn_TypeDef*)(pControl->objPTR);
			if(((GL_IconBtn_TypeDef*)pTmp)->EventHandler)
			{
				((GL_IconBtn_TypeDef*)pTmp)->EventHandler();
			}
      break;
    case GL_CUSTOM_BUTTON:
      pTmp = (GL_CustomBtn_TypeDef*)(pControl->objPTR);
			if(((GL_CustomBtn_TypeDef*)pTmp)->EventHandler)
			{
				((GL_CustomBtn_TypeDef*)pTmp)->EventHandler();
			}
      break;
    case GL_ICON_BTN_TEXT:
      pTmp = (GL_IconBtnText_TypeDef*)(pControl->objPTR);
			if(((GL_IconBtnText_TypeDef*)pTmp)->EventHandler)
			{
				((GL_IconBtnText_TypeDef*)pTmp)->EventHandler();
			}
      break;
    case GL_ICON_BTN_PUSH_PULL_TEXT:
      pTmp = (GL_IconBtnPushPullText_TypeDef*)(pControl->objPTR);
			if(((GL_IconBtnPushPullText_TypeDef*)pTmp)->EventHandler)
			{
				((GL_IconBtnPushPullText_TypeDef*)pTmp)->EventHandler();
			}
      break;
    case GL_RADIO_BUTTON:
      pTmp = (GL_RadioOption_TypeDef*)(pControl->objPTR);
			if(((GL_RadioOption_TypeDef*)pTmp)->EventHandler)
			{
    ...
    case GL_COMBOBOX:
      ...
      if (((GL_ComboBoxGrp_TypeDef*)(pTmp))->Secondary == GL_FALSE )
      {
        ...
        {}
				if(((GL_ComboBoxGrp_TypeDef*)(pTmp))->ComboOptions[index]->EventHandler)
				{
      ...
      else
      {
        ...
        {}
				if(pPrimaryOption->ComboOptions[index]->EventHandler)
				{
    ...
    case GL_SLIDEBAR:
      pTmp = (GL_Slidebar_TypeDef*)(pControl->objPTR);
			if(((GL_Slidebar_TypeDef*)pTmp)->EventHandler)
			{
...
/**
  * @brief  Initilize and change touchscreen settings.
  * @param  None
  * @retval None
  */
...
{
	if(_light_timeout_off > 0)
	{
		if (vu8_gSleepState == 0)
		{
			if (vu8_gPowerSaveOption == 1)
			{
				vu32_gTimeOutCount+=spend_time;
				if (vu32_gTimeOutCount > _light_timeout_off)
				{
					GL_BackLightSwitch(_light_deep_off);
					vu8_gSleepState = 1;
					vu32_gTimeOutCount = 0;
				}
			}
...
{
    _light_timeout_off = timeout;
}

void GL_SetBackLightDeep_Off(uint32_t deep)
{
    _light_deep_off = deep;
}
