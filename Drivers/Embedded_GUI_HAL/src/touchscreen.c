...
/** @defgroup touchscreen 
  ...
  */ 
/* External variables --------------------------------------------------------*/
extern int validTouchCalibration(void);
extern void Platform_SaveTouchCalibration(void *pData);
extern void Platform_ReadTouchCalibration(void *pData);
...
uint32_t EndAddr = 0;
volatile uint32_t TSC_Value_X;
volatile uint32_t TSC_Value_Y;
...
void TS_Calibration(void)
{
  ...
  uint32_t Xd1 = (LCD_Width / 2), Xd2 = 1 * (LCD_Width / 5), Xd3 = 4 * (LCD_Width / 5), Xd4 = 4 * (LCD_Width / 5), Xd5 = 1 * (LCD_Width / 5);
  uint32_t Yd1 = (LCD_Height / 2), Yd2 = 1 * (LCD_Height / 5), Yd3 = 1 * (LCD_Height / 5), Yd4 = 4 * (LCD_Height / 5), Yd5 = 4 * (LCD_Height / 5);
  ...
  GL_ClearScreen(WHITE);
  GL_DisplayAdjStringLine(LCD_Width/4, LCD_Height/2, "Touch the screen now", GL_FALSE);
  touch_done = 0;
  ...
  //GL_ClearScreen(WHITE);
	GL_DisplayAdjStringLine(LCD_Width/4, LCD_Height/2, "Calibration done!", GL_FALSE);
	GL_Delay(1000);
...
void TS_ReadCalibration(void)
{
  ...
  Platform_ReadTouchCalibration(Data);
	//calibration_magic = Data[0];
	A2 =  Data[1];
  B2 =  Data[2];
  C2 =  Data[3];
  D2 =  Data[4];
...
void TS_MakeCalibration(void)
{
	...
	Data[5] = E2;
	Data[6] = F2;
	Platform_SaveTouchCalibration(Data);
}
...
/**
  ...
  * @retval uint32_t - Xd: Coordinate X of the LCD Panel (X-axis Coordinate)
  */
...
/**
  ...
  * @param  x_touch: X coordinate of the Touch Panel
  * @param  y_touch: Y coordinate of the Touch Panel
  * @retval uint32_t - Yd: Coordinate X of the LCD Panel (Y-axis Coordinate)
  */
